...
class ImageConverter
{
  ...
  ros::Publisher image_pub = nh_.advertise<std_msgs::Int32MultiArray>("image_param", 100);
...
ImageConverter()
  ...
  {
    // カラー画像をサブスクライブ                                                                
    image_sub_ = it_.subscribe("/camera1/usb_cam1/image_raw", 1,
 ...
 // 入力パラメータ
	...
	cv::Rect range_Green(0, 0, 640,480);
...
//cv::Mat e = cv::getStructuringElement(cv::MORPH_RECT,cv::Size(3,3)); 
	//cv::morphologyEx(color_maskB, color_maskB, cv::MORPH_CLOSE, cv::Mat(), cv::Point(-1,-1), 1);
	//cv::threshold(color_maskB, color_maskB, 0, 255, cv::THRESH_BINARY);
...
//ウィンドウ表示
    ...
    cv::imshow("Blue Image", cv_half_imageB);
    cv::waitKey(3);
